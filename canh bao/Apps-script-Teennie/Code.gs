/**
 * Teennie Expense Alert System - Main Entry Point
...
function installTriggers() {
  try {
    ...
    if (config.TELEGRAM_CHAT_ID) {
      const message = `✅ *Teennie Expense Monitor - Triggers Installed*\n\n` +
...
function uninstallTriggers() {
  try {
    ...
    const config = getConfig();
    if (config.TELEGRAM_CHAT_ID) {
      const message = `⏸️ *Teennie Expense Monitor - Triggers Removed*\n\n` +
...
function initializeSystem() {
  try {
    log('=== Initializing Teennie Expense Monitor ===');
...
function triggerHealthCheck() {
  Logger.log('=== TRIGGER HEALTH CHECK (Teennie) ===');
  Logger.log('');
...
function debugListAllTriggers() {
  ...
  Logger.log('=== ALL TRIGGERS (Teennie) ===');
