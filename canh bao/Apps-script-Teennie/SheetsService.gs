...
/**
 * Get Report sheet
 * @returns {Sheet} Report_Teennie sheet
 */
function getReportSheet() {
  const ss = getSpreadsheet();
  const sheet = ss.getSheetByName('Report_Teennie');

  if (!sheet) {
    throw new Error('Sheet "Report_Teennie" not found. Please ensure the actual expense sheet is named "Report_Teennie".');
  }
...
function validateSheetStructure() {
  ...
  try {
    ...
    if (!reportSheet) {
      result.valid = false;
      result.errors.push('Report_Teennie sheet not found');
    }
